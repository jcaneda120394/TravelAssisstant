...
const PLACE_OR_FOOD =
  /\b(tokyo|osaka|kyoto|manila|malolos|cebu|bangkok|singapore|seoul|paris|london|rome|dubai|bali|restaurant|ramen|sushi|cafe|hotel)\b/i;

const OFF_TOPIC =
  /\b(homework|math|algebra|calculus|code this|write code|programming|python|javascript|react native|stock|crypto|bitcoin|nft|politics|election|dating advice|relationship advice|medical diagnosis|prescribe|lawsuit|legal advice|hack|exploit|weapon|bomb|nsfw|porn|gambling|casino|lottery|chatgpt|general knowledge|who is the president|write an essay)\b/i;
...
const REJECTION =
  'I only help with TravelAssistant topics near your current location — places, restaurants, hotels, routes, weather, budgets, translate, and trip planning. Ask something about nearby or your trip.';
...
export function evaluateTravelScope(question: string): TravelScopeResult {
  ...
  if (text.length < 2) {
    return {
      ok: false,
      reason: 'empty',
      rejectionMessage: 'Ask about nearby places, food, routes, hotels, or your trip plan.',
    };
  }
  ...
  // "What should we do this afternoon?" — weak cue alone is OK for nearby explore.
  ...
  return {
    ...
    reason: 'out_of_scope',
    rejectionMessage:
      'That looks outside TravelAssistant’s nearby scope. Try asking about places, restaurants, hotels, directions, weather, translate, or planning near your current city.',
  };
}
